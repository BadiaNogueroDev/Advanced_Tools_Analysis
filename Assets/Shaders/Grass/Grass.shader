Shader "Darian/Grass"
{
	Properties
	{
		...
		#include "Assets/Shaders/Grass/CustomTessellation.cginc"
		...
		//Añadido
#define MAX_TRAMPLES 100
		
		float _TrampleStrength;
		float4 _MultipleTramples[MAX_TRAMPLES];
		int _TramplesCount = 10;

#define BLADE_SEGMENTS 3

		float4 GetTrampleVector(float3 pos, float4 objectOrigin, float4 trample)
		{
			float3 trampleDiff = pos - (trample.xyz - objectOrigin);
			return float4(
		...
		void geo(triangle vertexOutput IN[3], inout TriangleStream<geometryOutput> triStream)
		{
			...
			for (int i = 0; i < BLADE_SEGMENTS; i++)
			{
				...
				//
				for (int n = 0; n < _TramplesCount; n++)
				{
					pos += GetTrampleVector(pos, objectOrigin, _MultipleTramples[n]) * _TrampleStrength;
				}
				//
			...
			for (int j = 0; j < _TramplesCount; j++)
			{
				pos += GetTrampleVector(pos, objectOrigin, _MultipleTramples[j]) * _TrampleStrength;
			}
